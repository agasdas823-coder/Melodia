...
export default function Explore() {
  ...
  useEffect(() => {
    const fetchNewReleases = async () => {
      try {
        const res = await axios.get("http://localhost:5002/api/search?q=top+hits&limit=10");
  ...
  const fetchSongs = useCallback(async () => {
    setLoading(true);
    try {
      let endpoint = "http://localhost:5002/api/search?q=top+hits+2024&limit=20";

      if (activeFilterTab === "genre" && selectedSubFilter) {
        endpoint = `http://localhost:5002/api/search?q=${encodeURIComponent(selectedSubFilter)}&limit=20`;
        ...
        endpoint = `http://localhost:5002/api/search?q=${encodeURIComponent(moodQuery)}&limit=20`;
      }
  ...
  const handleAddPlaylist = async (item, e) => {
    e.stopPropagation();
    try {
      const response = await axios.get(`http://localhost:5002/api/youtube-playlist/${item.id}`);
