...
function cleanYouTubeMusicTitle(value) {
  if (!value) return '';
  return String(value)
    .replace(/\s*\[[^\]]*\]/g, '')
    .replace(/\s*\([^)]*\)/g, '')
    .replace(/\b(official music video|official video|official audio|audio|lyric video|lyrics|music video)\b/gi, '')
    .replace(/\s{2,}/g, ' ')
    .trim();
}

function parseYouTubeMusicMetadata(title, channel) {
  const rawTitle = String(title || '').trim();
  const cleanedTitle = cleanYouTubeMusicTitle(rawTitle);
  const separators = [' - ', ' – ', ' — ', ' | ', ': '];
  ...
  for (const separator of separators) {
    ...
    const restJoined = rest.length ? `${second} ${separator} ${rest.join(` ${separator} `)}` : second;

    if (channel && first.toLowerCase().includes(channel.toLowerCase().slice(0, 10))) {
      return {
        title: `${second}${rest.length ? ` ${separator} ${rest.join(` ${separator} `)}` : ''}`.trim(),
        artist: first,
      };
    }
    ...
    return {
      title: first,
      artist: `${second}${rest.length ? ` ${separator} ${rest.join(` ${separator} `)}` : ''}`.trim(),
    };
  }

  if (channel && channel.toLowerCase() !== 'youtube') {
    return {
      title: cleanedTitle,
      artist: channel,
    };
...
function formatVideo(video) {
  ...
  const { title: parsedTitle, artist: parsedArtist } = parseYouTubeMusicMetadata(video.title, video.channel);
  const primaryArtist = parsedArtist || video.channel || 'YouTube';
  const primaryTitle = parsedTitle || video.title;
